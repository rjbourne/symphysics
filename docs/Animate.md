...
We begin with a setup similar to that in `pendulum.py` from SystemL example 1
...
```
We wish to add two particles, a pivot and a pendulum bob, so we define functions to take the generalised coordinates and return [x,y] corrdinates:
